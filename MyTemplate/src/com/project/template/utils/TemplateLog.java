...
public class TemplateLog {
    ...
    // 设置当前的LogLevel
    // Log错误界别越高，值越大，如果设置的值越低，则打印的Log信息也多，如果设置backtrace为True，则打印栈信息
    public static void setCurLogLevel(int curLogLevel) {
        TemplateLog.curLogLevel = curLogLevel;
    }
    ...
    public synchronized static void initTrace(String msg, int... isPrintStack) {
        ...
        msgT = "[" + className + ":" + currentThread[curentIndex].getLineNumber() + ":" + currentThread[curentIndex].getMethodName() + "()]---";
        msgC = "msg:[" + msg + "]";
        if (isPrintStackOne > 0) {
            ...
            for (int i = curentIndex; i < curentIndex + isPrintStackOne && i < currentThread.length; i++) {
                sb.append(currentThread[i].getFileName() + ":" + currentThread[i].getLineNumber() + ":" + currentThread[i].getMethodName() + "()" + "<--");
            }
            sb.append("]");
